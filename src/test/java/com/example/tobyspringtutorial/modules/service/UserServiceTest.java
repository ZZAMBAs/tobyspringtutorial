...
class UserServiceTest {
    ...
    public void setUp(){
        users = Arrays.asList( // 배열을 리스트로 만들어주는 편리한 메서드. 배열을 가변 인자로 넣어주면 더욱 편리.
                ...
                new User("EH", "엘링 홀란", "p1", Level.BASIC,
                        MIN_LOGCOUNT_FOR_SILVER - 1, 0), // 경곗값 분석 테스트
                new User("MS", "모하메드 살라", "p2", Level.BASIC,
                        MIN_LOGCOUNT_FOR_SILVER, 0),
                new User("SHM", "손흥민", "p3", Level.SILVER,
                        60, MIN_RECCOUNT_FOR_GOLD - 1),
                new User("MR", "마커스 래시포드", "p4", Level.SILVER,
                        60, MIN_RECCOUNT_FOR_GOLD),
                new User("KDB", "케빈 데브라위너", "p5", Level.GOLD,
    ...
    void checkLevelUpgradeOccurred(User user, boolean isUpgraded){ // 리팩토링. 이제 2번째 파라미터는 업그레이드 되어야 하는지를 넘겨준다.
